...
import { buildYpick } from "./situation.js";
  ...
  state.nwHistory.forEach((r, i) => {
    ...
    row.innerHTML =
      ...
      '<div class="inp cur mini"><span class="pre">€</span>' +
        '<input type="number" min="0" step="1000" placeholder="Net worth" value="' + fv(r.val) + '"></div>' +
      ...
      '<div class="dp-extra">' +
        '<div class="inp cur mini"><span class="pre">€</span><input type="number" min="0" step="100" placeholder="Income /mo" value="' + fv(r.income) + '"></div>' +
        '<div class="inp cur mini"><span class="pre">€</span><input type="number" min="0" step="100" placeholder="Spending /mo" value="' + fv(r.spend) + '"></div>' +
      '</div>';
    ...
    valEl.addEventListener('input', e => { const n = parseFloat(e.target.value); r.val = isNaN(n) ? 0 : n; recalc(); });
    incEl.addEventListener('input', e => { const n = parseFloat(e.target.value); r.income = isNaN(n) ? null : n; recalc(); });
    spendEl.addEventListener('input', e => { const n = parseFloat(e.target.value); r.spend = isNaN(n) ? null : n; recalc(); });
